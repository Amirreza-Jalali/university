...
lower_color = np.array([127, 40, 70])
upper_color = np.array([0, 255, 255])

mask = cv2.inRange(hsv, lower_color, upper_color)

kernel = np.array((5,5), np.unit8)

mask = cv2.erode(mask, kernel, iterations=1)
mask = cv2.dilate(mask, kernel, iterations=1)

contures, _ = cv2.findContours(mask,cv2.RETR_TREE, CHAIN_APPROX_SIMPLE)
print(_)

conclution = image.copy()

cv2.drawContours(conclution, contours, -1, (0, 255, 0), 2)

concat = cv2.honocat([image, conclution])
